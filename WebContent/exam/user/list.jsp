...
<html>
	...
	<body>
		...
		<div class="footer">
			<button onclick="addData()">添加用户</button>
		...
		<form name="deleteForm" action="deleteUser" method="get">
	...
	<script type="text/javascript">
		...
		<%for(int i = 0; i < list.size(); i++) {
			...
			if(i != list.size() - 1){%>
				{code: "<%=user.getCode()%>", userName: "<%=user.getUserName()%>", realName: "<%=user.getRealName()%>", 
					gender: "<%=user.getGender()%>", identity: "<%=user.getIdentity()%>"},
			<%}else{%>
				{code: "<%=user.getCode()%>", userName: "<%=user.getUserName()%>", realName: "<%=user.getRealName()%>", 
					gender: "<%=user.getGender()%>", identity: "<%=user.getIdentity()%>"}
		<%}}%>
		...
		function getDataRow(h) {
			...
			identityCell.innerHTML = h.identity;
		 ...
		 */
		function addData() {
			open("add.jsp", "", "width=650, height=500");
		}
